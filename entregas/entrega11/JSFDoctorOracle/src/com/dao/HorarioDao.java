...
public class HorarioDao implements HorarioService {
	...
	@Override
	public List<Doctor> getDoctoresFilter(String caracter) {
		String sql = "SELECT id_doctor, nombre, apellidos, direccion, telefono, rfc, especialidad, sexo, correo, contra from doctor where nombre like ?";
		Connection conn = null;

		List<Doctor> listDocs = new ArrayList<Doctor>();
		try {
			conn = dataSource.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			...
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				listDocs.add(new Doctor(rs.getInt("id_doctor"), rs.getString("nombre"), rs.getString("apellidos"),
